<template>
  <section class="get-list-container w-full grid justify-items-center">
    ...
    <h4 class="text-h4 m-4 text-blue-600">
      Ou faça uma busca por países ou um país em questão
    </h4>
    ...
    <div class="fields-content grid w-2/6">
      <v-card-text class="">
        ...
        <v-select
          ...
          :items="[
            { title: '', lang: '' },
            { title: 'Português', lang: 'portuguese' },
            { title: 'Inglês', lang: 'english' },
            { title: 'Espanhol', lang: 'spanish' },
            { title: 'Francês', lang: 'french' },
          ]"
        ></v-select>
        <v-btn variant="tonal" color="blue-darken-2" @click="searchByLanguage">
          Pesquisar
        </v-btn>
      </v-card-text>
      ...
      <v-card-text>
        ...
        <v-text-field
          density="compact"
          variant="solo"
          label="Digite o nome de um país em inglês"
    ...
    <div v-if="isVisible == true" class="w-9/12">
      <div class="w-full">
        <iframe
          width="100%"
          height="250"
